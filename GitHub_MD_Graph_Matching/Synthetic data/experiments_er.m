%synthetic data experiments with the Correlated Erdos-Renyi model. 
% We compare MDGM,PDGGM and Grampa.
%   n_exp = identifier to the save the results of the experiments.IMPORTANT: it overwrites if not changed
%   m_iter = maximum MD iterations
%   n = size of the graph
function []=experiments_er(n_exp,m_iter,n)

addpath(genpath('Helpers'));
addpath(genpath('Algos Graph Matching'));
p = .5;
num_run = 15; %number of Monte Carlo runs
vec_noise = 0:0.05:0.8;%grid of noise parameters
len_noise = length(vec_noise);
maxiter = m_iter;% (log(n)^3);%maximum number of MD or GD iterations
%
...
for ind_run = 1:num_run
    fprintf('Iteration %i \n', ind_run);
        ...
        for ind_noise = 1:len_noise
            ...
            [A, B, ~, ~, P_rnd] = generate_er(n, p, sigma);%sample ER graphs
            %% MD
            clock_md = tic;
            %initial=(1/n)*ones(n);
            P = matching_MD(A,B,maxiter,'dynamic_steps');
            ...
            run_gr(ind_noise, ind_run) = toc(clock_grampa);
             ...
             %% PGD
            clock_pgd = tic;
            P = matching_PGD(A,B,maxiter,'heuristic');
            fix_pt_ratio = sum(dot(P_rnd, P)) / n;
...
% compute the std running time over Monte Carlo runs
...
save(id_savedvars,'run_md','run_pgd','run_md_mean','run_pgd_mean','run_md_std','run_pgd_std' ,'run_gr_mean','run_gr_std');
end 
